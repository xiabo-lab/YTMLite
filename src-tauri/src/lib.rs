...
async fn set_cache_dir(
    ...
    match path {
        ...
        Some(raw) => {
            ...
            let probe = dir.join(".ytmlite-write-test");
...
fn build_tray(app: &tauri::AppHandle) -> tauri::Result<()> {
    let show_item = MenuItem::with_id(app, "show", "Show YTMLite", true, None::<&str>)?;
...
pub fn run() {
    ...
    // media tile (and notifications, taskbar) resolve to "YTMLite" + icon rather
